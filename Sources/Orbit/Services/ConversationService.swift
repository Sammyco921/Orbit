...
final class ConversationService {
    ...
    private let maxMessagesBeforeSummarize = 50
    ...
    func addMessage(_ message: Message) -> Int? {
        ...
        // Trigger context management if conversation is growing too long
        if conversations[index].messages.count >= maxMessagesBeforeSummarize {
            condenseConversationIfNeeded(convIndex: index)
        }
        return index
    }

    /// Summarize older messages when the conversation exceeds the threshold.
    /// Keeps the last 20 messages intact, summarizes everything before them.
    private func condenseConversationIfNeeded(convIndex: Int) {
        let keepCount = 20
        let msgs = conversations[convIndex].messages
        guard msgs.count >= maxMessagesBeforeSummarize + keepCount else { return }

        let toSummarize = Array(msgs.dropLast(keepCount))
        let toKeep = Array(msgs.suffix(keepCount))

        guard let llm = llmService?.currentProvider() else {
            // If no LLM, just truncate
            conversations[convIndex].messages = toKeep
            saveConversations()
            return
        }
        ...
        Task {
            do {
                let summary = try await summarizer.summarize(messages: toSummarize, provider: llm)
                let summaryMsg = Message(role: .system, content: "[Earlier conversation summary: \(summary)]")
                await MainActor.run {
                    conversations[convIndex].messages = [summaryMsg] + toKeep
                    saveConversations()
                }
            } catch {
                log.warning("Summarization failed, truncating: \(error.localizedDescription)")
                await MainActor.run {
                    conversations[convIndex].messages = toKeep
                    saveConversations()
                }
