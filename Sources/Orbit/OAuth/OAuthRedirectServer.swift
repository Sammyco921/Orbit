...
actor OAuthRedirectServer {
    ...
    func start(port: UInt16) async throws -> Int {
        ...
        let usePort: NWEndpoint.Port
        if port == 0 {
            usePort = NWEndpoint.Port(rawValue: UInt16.random(in: 49152...65535))!
        } else {
            guard let p = NWEndpoint.Port(rawValue: port) else {
                throw OAuthError.redirectServerFailed("Invalid port: \(port)")
            }
            usePort = p
        }
        let listener = try NWListener(using: params, on: usePort)
