...
final class JobStore {
    ...
    private func jobFromRow(_ row: Row) -> ExecutionJob {
        ExecutionJob(
            jobId: UUID(uuidString: row["jobId"] as! String)!,
            storyId: UUID(uuidString: row["storyId"] as! String)!,
            intent: row["intent"] as! String,
            state: JobState(rawValue: row["state"] as! String) ?? .created,
            createdAt: Date(timeIntervalSince1970: row["createdAt"] as! Double),
            updatedAt: Date(timeIntervalSince1970: row["updatedAt"] as! Double),
            currentStepIndex: row["currentStepIndex"] as! Int,
            executionMode: ExecutionMode(rawValue: row["executionMode"] as! String) ?? .interactive,
