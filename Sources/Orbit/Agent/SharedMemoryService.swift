...
private struct AnyCodableWrapper: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        else if let arr = try? container.decode([String].self) { value = arr }
    ...
    func encode(to encoder: Encoder) throws {
        ...
        else if let arr = value as? [String] { try container.encode(arr) }
        else if let dict = value as? [String: String] { try container.encode(dict) }
