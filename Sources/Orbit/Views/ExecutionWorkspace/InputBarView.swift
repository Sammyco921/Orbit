...
struct InputBarView: View {
    ...
    private var multiAgentToggle: some View {
        Button {
            uxOrchestrator?.useMultiAgent.toggle()
        } label: {
            Image(systemName: uxOrchestrator?.useMultiAgent == true ? "person.3.fill" : "person.3")
                .font(.system(size: 18))
                .foregroundStyle(uxOrchestrator?.useMultiAgent == true ? .orbitAccent : .orbitTertiary)
        }
        .buttonStyle(.plain)
        .help(uxOrchestrator?.useMultiAgent == true ? "Multi-agent mode (on)" : "Single-agent mode")
        .disabled(isExecuting)
    }
